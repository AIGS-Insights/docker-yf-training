...
SEED_ZIP="/tmp/training-configdb.zip"
DATA_DIR="/opt/yellowfin"
MARKER_FILE="$DATA_DIR/data/configdb.data"
if [ -f "$SEED_ZIP" ] && [ ! -f "$MARKER_FILE" ]; then
  if [ -z "$(ls -A "/opt/yellowfin/data/" 2>/dev/null)" ]; then
    echo "Seeding embedded DB..."
    unzip -o "$SEED_ZIP" -d "$DATA_DIR"
    sync || true
  else
    echo "Data dir not empty; skipping seed."
    echo "$(ls -A "/opt/yellowfin/data/" 2>/dev/null)"
  fi
fi
